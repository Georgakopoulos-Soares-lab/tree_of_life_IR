...
def distance_from_origin(
    intersect_df: pd.DataFrame, polynomial: Callable, window_size: int, step: int = 500
    ...
    intersect_df["distance_bin"] = intersect_df["distance"].apply(
        lambda x: x // step + 1
    )
    counts_per_bin = intersect_df.groupby("distance_bin").agg(
        totalCounts=("seqID", "count"),
    )
...
def bootstrap(
    ...
    for _ in range(N):
        ...
        density = extractor.extract_density(
            sample_df, window_size=window_size, return_array=True, enrichment=True
        )
...
class DensityExtractor:
    ...
    biotypes: Optional[list[str]] = field(
        factory=lambda: ["protein_coding", "non_coding", "."]
    )
    extractions: dict[str, str] = field(init=False, repr=False)
    window_size: int = field(
        default=500, converter=int, validator=attr.validators.instance_of(int)
    )
    ...
    def __attrs_post_init__(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                f"Invalid parquet compression selected: `{self.compression}`."
            )
    ...
    def preprocess_extraction_df(
        ...
        if (
            "seqID" not in extractions_df.columns
            and "chromosome" in extractions_df.columns
        ):
            extractions_df = extractions_df.rename({"chromosome": "seqID"})
        elif (
            "seqID" not in extractions_df.columns and "chrom" in extractions_df.columns
        ):
            extractions_df = extractions_df.rename({"chrom": "seqID"})
        elif (
            "seqID" not in extractions_df.columns
            and "sequence_name" in extractions_df.columns
        ):
            ...
            raise KeyError(
                f"Column `seqID` is not present in the extractions dataframe."
            )
        ...
        if "end" not in extractions_df:
            raise KeyError(
                f"Column `end` is not present in the extractions dataframe ({extraction})."
            )

        # Check if polarity can be determined correctly
        if (
            "motif_strand" not in extractions_df.columns
            and "strand" not in extractions_df.columns
            and "sequence" not in extractions_df.columns
            and self.mode == "polarity"
        ):
            raise PolarityError(
                f"Failure to detect polarity. Either `strand` or `sequence` must be present in the dataframe when selected mode is `template`."
            )
        elif (
            "strand" not in extractions_df
            and "motif_strand" not in extractions_df.columns
            and self.mode == "polarity"
        ):
    ...
    def parse_density(
        ...
        new_columns = (
            A_columns + ["chrom", "motif_start", "motif_end"] + B_columns + ["overlap"]
        )
        try:
        ...
        matched_queries = (
            ...
            .with_columns(
                (pl.col("match_bp") > 0).cast(pl.Int8).alias("atLeastOneMatch")
            )
        ...
        total_matched_targets = intersect_df.unique(
            subset=["chrom", "motif_start", "motif_end"]
        ).shape[0]
        matched_targets_template: Optional[float]
        matched_targets_non_template: Optional[float]
        if self.mode == "polarity":
            if intersect_df.shape[0] > 0:
                intersect_df = intersect_df.filter(
                ...
                matched_targets_homopolarity = round(
                    1e2
                    * intersect_df.with_columns(
                ...
                # template proportion
                matched_targets_heteropolarity = round(
                    1e2 - matched_targets_homopolarity, 2
                )
            else:
        ...
        target_perc: Optional[float] = (
            round(1e2 * total_matched_targets / total_targets, 2)
            if total_targets > 0
            else float("nan")
        )
        query_perc: Optional[float] = (
            round(1e2 * total_matched_queries / total_queries, 2)
            if total_queries > 0
            else float("nan")
        )
        ...
        pwm = PWMExtractor()
        if self.mode == "polarity":
            density = pwm.extract_template_density(
                intersect_df,
        ...
        if self.mode == "polarity":
            density_df = pl.DataFrame(
                {
                    ...
                    **{
                        str(int(i) - self.window_size): [
                            density["Template"][i],
                            density["Non-Template"][i],
                        ]
            ...
            density_df = pl.DataFrame(
                {
                    **{
                        str(int(i) - self.window_size): [density[i]]
                        for i in range(2 * self.window_size + 1)
                    },
    ...
    def process_site(
        self,
        extractions_table: pl.DataFrame,
        ...
        if loci != "start" and loci != "end":
            raise ValueError(
                f"Invalid transcription site loci `{loci}`. Select either `start` or `end`."
            )
        ...
        for biotype in self.biotypes:
            ...
            if gff_expanded_temp.shape[0] == 0:
                logging.warning(
                    f"GFF file `{accession_id}` for biotype `{biotype}` is empty."
                )
                continue
            ...
            site_density = self.parse_density(
                ...
                merge_B=merge_motifs,
                enrichment=enrichment,
            ).with_columns(pl.lit(biotype).alias("biotype"), pl.lit(site).alias("site"))
            if partition_col:
                site_density = site_density.with_columns(
    ...
    def process(
        ...
        try:
            extractions_df = pl.read_csv(
                extraction, separator=delimiter, comment_prefix="#"
            )
            ...
            logging.info(
                f"Invalid delimiter. Accession ID `{accession_id}` was found empty.\n{e}"
            )
            is_empty = True
        ...
        if is_empty:
            ...
            self.empty_accessions.add(accession_id)
            # return densities_df
        extractions_df = self.preprocess_extraction_df(
            extractions_df, polarity_mode=polarity_mode, partition_col=partition_col
        )
        # Check if there are partitions
        if partition_col is not None and not isinstance(partition_groups, list):
            raise TypeError(
                f"Partition groups must be a list when a partition column `{partition_col}` has been chosen."
            )
        if partition_col is not None and len(partition_groups) == 0:
            raise ValueError(
                "Partition groups cannot be an empty list when a partition column `{partition_col}` has been chosen."
            )
        if partition_col is not None and partition_col not in extractions_df:
            raise KeyError(
                f"Column `{partition_col}` is not present in the extractions dataframe ({extraction})."
            )

        # Continue with density extraction
        gff_df = gff_reader.read_gff(
            gff_file,
            change_names=True,
            end_one_base=True,
        ...
        if gff_df is None or gff_df.shape[0] == 0:
            logging.warning(
                f"Failed to process gff file `{gff_file}`. Reason: no records of `{compartment}` were found in the annotation file."
            )
        ...
        for loci in self.transcription_site_loci:
            densities_table = self.process_site(
                extractions_table=extractions_df,
                gff_table=gff_df,
                loci=loci,
                enrichment=enrichment,
                return_df=False,
    ...
    def process_bucket(
        ...
        daemon = threading.Thread(
            target=tracker.start, daemon=True, name="LoggingDensityDaemon"
        )
        daemon.start()
        ...
        if self.mode == "density":
            for site in sites:
                for _biotype in self.biotypes:
                    collection_dest[site, _biotype] = (
                        f"{out}/enrichment_bucket_{bucket_id}_{window_size}_{compartment}_{self.mode}_{polarity_mode}.{_biotype}.{site}.{self.format}"
                    )
            ...
            for site in sites:
                for _biotype in self.biotypes:
                    if _biotype == ".":
                        _biotype = "generic"
                    collection_dest[site, _biotype] = (
                        f"{out}/enrichment_bucket_{bucket_id}_{window_size}_{compartment}_{self.mode}_{polarity_mode}.{_biotype.replace('.', 'generic')}.{site}.{self.format}"
                    )
            dest_empty = f"{out}/empty_accessions_bucket_{bucket_id}_{window_size}_{compartment}_{self.mode}_{polarity_mode}.txt"
        # # Initialize Collection # #
        for site in sites:
            for _biotype in self.biotypes:
                collection_handlers[site, _biotype] = open(
                    collection_dest[site, _biotype], mode="w", encoding="UTF-8"
                )
                first_headers[site, _biotype] = True
                at_ = collection_dest[site, _biotype]
                logging.info(
                    f"Saving density output for {site} & biotype={_biotype} at `{at_}`... (bucket {bucket_id})."
                )
                print(
                    colored(
                        f"Saving density output for {site} & biotype={_biotype} at `{at_}`... (bucket {bucket_id}).",
            ...
            if EXCEEDED_STREAMING_LOAD:
                ...
                for site in sites:
                    for _biotype in self.biotypes:
                        (
                            densities_df.filter(
                                pl.col("site") == site, pl.col("biotype") == _biotype
                            ).write_csv(
        ...
        for site in sites:
            ...
            logging.info(
                f"Closing file for {site} & biotype={_biotype} at `{at_}`... (bucket {bucket_id})."
            )
            print(
                colored(
                    f"Closing file for {site} & biotype={_biotype} at `{at_}`... (bucket {bucket_id}).",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="""Extracts motif density across genomic subcompartment of interest around a specified window."""
    )
    ...
    parser.add_argument(
        "--mode", "-m", type=str, choices=["polarity", "density"], default="density"
    )
    parser.add_argument("--partition_col", default=None)
    parser.add_argument(
        "--partition_groups", default=list(range(0, 9)), type=list, nargs="+"
    )
    parser.add_argument("--float_precision", type=int, default=2)
    parser.add_argument(
        "--polarity_mode", default="GC", choices=["GC", "GA", "GT"], type=str
    )
    parser.add_argument(
        "--compartment", "-c", type=str, choices=["gene", "exon"], default="gene"
    )
    ...
    if not Path(args.tempdir).is_dir():
        raise ValueError(f"Temporary directory {args.tempdir} does not exist.")
    ...
    if args.partition_col:
        ...
        biolog_file = out.joinpath(
            f"biologs/enrichment_bucket_{bucket_id}_mode_{mode}_partition_{args.partition_col}.log"
        )
    else:
        out = out.joinpath(f"mode_{mode}")
        biolog_file = out.joinpath(
            f"biologs/enrichment_bucket_{bucket_id}_mode_{mode}.log"
        )
