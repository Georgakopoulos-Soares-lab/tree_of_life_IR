...
def bootstrap_density(
    ...
    for _ in tqdm(range(nsamples), leave=True, position=0):
        ...
        df_sample = extractor.extract_density(
            df_sample, window_size=window_size, enrichment=True, return_array=True
        )
...
class Bootstrapper:
    def __init__(
        ...
        if not self.enrichment_file.is_file():
            raise FileNotFoundError(
                f"Could not detect enrichment file `{enrichment_file}`."
            )
    ...
    def load_table(self, taxonomic_ranks: Optional[list[str]] = None):
        ...
        if not isinstance(taxonomic_ranks, list):
            raise TypeError(
                f"Invalid type for taxonomic ranks. Expected list, but received {type(taxonomic_ranks)}."
            )
        ...
        design_df = pl.read_csv(self.design, columns=columns, separator=delimiter)
        ...
        enrichment_df = enrichment_df.join(
            design_df,
    ...
    def bootstrap_enrichment(
        ...
        if (partition_col is None and partition_list is not None) or (
            partition_col is not None and partition_list is None
        ):
            raise TypeError(
                "Partition column and partition list variables must simultaneously be None."
            )
        enrichment_df = enrichment_df.filter(pl.col(taxonomic_rank) == rank)

        print(
            f"Initializing bootstrap for taxonomic rank {taxonomic_rank} with value=`{rank}`."
        )
        ...
        if enrichment_df.shape[0] == 0:
            raise ValueError(
                f"Empty dataframe for taxonomic rank `{taxonomic_rank}` with value=`{rank}`."
            )
        selected_columns = ["biotype"]
        if partition_col and "." not in partition_list:
            print(
                f"Chosen partition on column `{partition_col}'. Partition values: {partition_list}."
            )
        ...
        if polarity:
            ...
            polarities = [None]
        combinations = [
            (polarity, biotype, partition_value)
        ...
        for comb in combinations:
            ...
            for level in self.bootstrap_levels:
                ...
                if merge_species_taxid:
                    ...
                    temp_df_agg = (
                        temp_df.group_by(level, maintain_order=True)
                        .agg(
                            *[
                                pl.col(str(i)).sum()
                                for i in range(
                                    -params.window_size, params.window_size + 1
                                )
                            ]
                        )
                        .select(
                    ...
                    print(
                        f"Merging species taxid. Total rows prior to reduction {before}. Total rows after reduction: {temp_df_agg.shape[0]}."
                    )
                else:
                    temp_df_agg = temp_df.select(
                        [
                            str(i)
                            for i in range(-params.window_size, params.window_size + 1)
                        ]
                    )
                average, lower_ci, upper_ci = bootstrap(
                    temp_df_agg, N=params.N, alpha=params.alpha
                )
                ...
                for i in range(2 * params.window_size + 1):
                    ...
                    confidence_intervals[str(i - params.window_size)].append(
                        lower_ci[i]
                    )
                    confidence_intervals[str(i - params.window_size)].append(
                        upper_ci[i]
        ...
        confidence_intervals = pl.DataFrame(confidence_intervals).select(
            [taxonomic_rank, "statistic", "bootstrap_level"]
            + selected_columns
            + list(map(str, range(-params.window_size, params.window_size + 1)))
        )
        confidence_intervals.write_csv(
            output,
            separator=",",
            include_header=True,
        ...
        print(
            colored(
                f"Bootstrap has succesfully been completed for taxonomic rank {taxonomic_rank}=`{rank}`.",
                "green",
            )
        )
    ...
    def average_phylums(
        self, domain: str, output: str, join_templates: int = 0
    ) -> None:
        self.load_table()
        ...
        phylum_to_kingdom = dict(
            zip(enrichment_df_phylum["phylum"], enrichment_df_phylum["kingdom"])
        )
        ...
        if "strand_polarity" in enrichment_df_phylum and not join_templates:
            ...
            enrichment_df_phylum = enrichment_df_phylum.groupby(
                ["phylum", "strand_polarity", "biotype"]
            )
        else:
        ...
        enrichment_df_phylum = enrichment_df_phylum.agg(
            {str(i): "sum" for i in range(-params.window_size, params.window_size + 1)}
        )
        enrichment_df_phylum = enrichment_df_phylum.apply(
        ...
        enrichment_df_phylum.loc[:, "kingdom"] = enrichment_df_phylum["phylum"].map(
            phylum_to_kingdom
        )
        ...
        print(
            colored(
                f"Phylum averaging has succesfully been completed for domain=`{domain}`.",
                "green",
            )
        )
        return
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="Bacteria",
        choices=["Eukaryota", "Archaea", "Viruses", "Bacteria"],
    )
    ...
    bootstrapper.bootstrap_enrichment(
